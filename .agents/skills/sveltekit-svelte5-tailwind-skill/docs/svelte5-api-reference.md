...
## Stores (Legacy — Avoid in New Code)

Svelte 5 runes replace stores. **Do not use stores in new Svelte 5 code.** Prefer `$state()` and `$derived()` instead. The following is documented for reference only when interoperating with legacy code.

```js
// ❌ Legacy — avoid in new Svelte 5 code
...
**Auto-subscription in components (legacy):**
```svelte
<script>
	// ❌ Legacy — avoid in new Svelte 5 code
